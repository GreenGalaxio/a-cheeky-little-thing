package com.mcleodgaming.ssf2.platforms
{
   ...
   public class Bumper1_T extends CNZBumper
   {
       
      
      public function Bumper1_T(param1:MovieClip, param2:StageData)
      {
         super(param1,param2);
      }
      ...
      override public function extraHitTests(param1:Number, param2:Number, param3:InteractiveSprite = null) : Boolean
      {
         ...
         if(Utils.fastAbs(m_attack.AttackID - _loc4_.PreviousAttackID) > 1 && !_loc4_.IsCaught && _loc6_.intersects(_loc5_))
         {
            ...
            {
               bouncePlayer(_loc4_,90);
            }
            else if(_loc4_.Y >= m_platform.y && _loc4_.X < m_platform.x)
            {
               bouncePlayer(_loc4_,225);
            }
            else if(_loc4_.Y >= m_platform.y && _loc4_.X >= m_platform.x)
            {
               bouncePlayer(_loc4_,315);
            }
            return true;
         }
         return false;
      ...
      override protected function checkBounce() : void
      {
         ...
         while(_loc5_ < m_opponents.length)
         {
            ...
            if(_loc2_ != null)
            {
               ...
               {
                  if(_loc2_.Y < m_platform.y)
                  {
                     bouncePlayer(_loc2_,90);
                  }
                  else if(_loc2_.Y >= m_platform.y && _loc2_.X < m_platform.x)
                  {
                     bouncePlayer(_loc2_,225);
                  }
                  else if(_loc2_.Y >= m_platform.y && _loc2_.X >= m_platform.x)
                  {
                     bouncePlayer(_loc2_,315);
                  }
               }
            }
            _loc5_++;
