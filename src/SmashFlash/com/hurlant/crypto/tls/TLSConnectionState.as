   ...
   public class TLSConnectionState implements IConnectionState
   {
      ...
      public function decrypt(param1:uint, param2:uint, param3:ByteArray) : ByteArray
      {
         ...
         if(this.macAlgorithm != MACs.NULL)
         {
            ...
            _loc6_ = param3.length - this.hmac.getHashSize();
            _loc5_.writeUnsignedInt(this.seq_hi);
            _loc5_.writeUnsignedInt(this.seq_lo);
            _loc5_.writeByte(param1);
            _loc5_.writeShort(TLSSecurityParameters.PROTOCOL_VERSION);
            _loc5_.writeShort(_loc6_);
            if(_loc6_ != 0)
            {
               _loc5_.writeBytes(param3,0,_loc6_);
            }
            _loc7_ = this.hmac.compute(this.MAC_write_secret,_loc5_);
            _loc8_ = new ByteArray();
            _loc8_.writeBytes(param3,_loc6_,this.hmac.getHashSize());
      ...
      public function encrypt(param1:uint, param2:ByteArray) : ByteArray
      {
         ...
         if(this.macAlgorithm != MACs.NULL)
         {
            ...
            _loc4_.writeShort(TLSSecurityParameters.PROTOCOL_VERSION);
            _loc4_.writeShort(param2.length);
            if(param2.length != 0)
            {
               _loc4_.writeBytes(param2,0,param2.length);
            }
            _loc3_ = this.hmac.compute(this.MAC_write_secret,_loc4_);
